...
class App {
  ...
  async loadPageModules() {
    if (!this.state.pagesConfig?.pages) return;
      ...
      return new Promise((resolve, reject) => {
        ...
        script.onload = () => {
          if (!window[page.module]) {
            reject(new Error(`页面模块未注册: id=${page.id}, module=${page.module}, file=${page.file}`));
            return;
          }
          resolve();
        };
    ...
    if (errors.length > 0) {
      const error = new Error(`加载页面模块出错:\n${errors.join('\n')}`);
  ...
  renderUI(container, template, data = {}, options = {}) {
    ...
    if (data && typeof template === 'string') {
      // 替换 ${key} 形式的变量
      processedTemplate = template.replace(/\${([^}]+)}/g, (_match, key) => {
        // 支持简单的表达式计算
        try {
          // 创建一个带有数据对象属性的上下文
          const context = Object.assign({}, data, {
            I18n: window.I18n || { translate: (_key, fallback) => fallback }
          });
  ...
  processEventBindings(element, context = {}) {
    ...
    const eventElements = element.querySelectorAll(
      '[data-on-click], [data-on-change], [data-on-input], [data-on-submit]'
    );
  ...
  static configureRendering(config) {
    Object.assign(App.renderConfig, config);
  }
  ...
  static batchRender(elements, renderCallback) {
    const batchSize = App.renderConfig.batchSize;
    let index = 0;
...
// biome-ignore lint/complexity/noStaticOnlyClass: PreloadManager 设计为纯静态工具类
class PreloadManager {
  ...
  static registerDataLoader(pageName, loader) {
    ...
    PreloadManager.loadingPromises.set(pageName, loader);
  }
  ...
  static async preloadData(pageName) {
    if (PreloadManager.dataCache.has(pageName)) return PreloadManager.dataCache.get(pageName);

    const loader = PreloadManager.loadingPromises.get(pageName);
    if (!loader) return null;

    try {
      const data = await loader();
      PreloadManager.dataCache.set(pageName, data);
      return data;
    } catch (error) {
  ...
  static getData(pageName) {
    return PreloadManager.dataCache.get(pageName);
  }
  ...
  static clearCache(pageName) {
    PreloadManager.dataCache.delete(pageName);
  }
}
// 路由管理器
// biome-ignore lint/complexity/noStaticOnlyClass: Router 设计为纯静态工具类
class Router {
  // 页面模块映射
  static modules = {};
  ...
  static initModules(config) {
    ...
    config.pages.forEach(page => {
      Router.modules[page.id] = page.module;
    });
  }
  ...
  static getCurrentPage() {
    ...
    return Router.modules[hash] ? hash : app.state.pagesConfig?.defaultPage || 'status';
  }
  ...
  static async preloadPage(pageName) {
    if (Router.cache.has(pageName)) return;
      ...
      tasks.push(
        (async () => {
          try {
            ...
            Router.cache.set(pageName, pageModule);
          } finally {
    ...
    // 预加载页面数据
    if (Router.preloadConfig.preloadData && pageModule.preloadData) {
  ...
  static preloadPages() {
    if (!app.state.pagesConfig?.pages) return;

    const { batchSize, timeout } = Router.preloadConfig;
    const currentPage = app.state.currentPage;

    // 获取要加载的页面
    const pagesToLoad = app.state.pagesConfig.pages.map(page => page.id).filter(pageId => pageId !== currentPage);

    const preloadBatch = async startIndex => {
      const batch = pagesToLoad.slice(startIndex, startIndex + batchSize);
      if (batch.length === 0) return;

      await Promise.allSettled(batch.map(page => Router.preloadPage(page)));

      if (startIndex + batchSize < pagesToLoad.length) {
        requestIdleCallback(() => preloadBatch(startIndex + batchSize), { timeout });
      }
    };
  ...
  static async navigate(pageName, updateHistory = true, isInitialLoad = false) {
    try {
      ...
      if (app.state.currentPage) {
        const currentPageModule =
          Router.cache.get(app.state.currentPage) || window[Router.modules[app.state.currentPage]];
      ...
      // 加载新页面模块
      let pageModule;
      if (Router.cache.has(pageName) && Router.cache.get(pageName)) {
        pageModule = Router.cache.get(pageName);
      } else {
        pageModule = window[Router.modules[pageName]];
      ...
      UI.showError('页面加载失败', error);
    }
  }

  // 页面过渡效果
  static async performPageTransition(newContainer, oldContainer, _newPageName) {
    // 如果没有旧容器，直接显示新容器
    if (!oldContainer) {
      requestAnimationFrame(() => {
        newContainer.classList.add('slide-in');
...
// UI管理器
// biome-ignore lint/complexity/noStaticOnlyClass: UI 设计为纯静态工具类
class UI {
  // UI元素引用
  static elements = {
  ...
  static updatePageTitle(pageName) {
    ...
    const title = pageConfig.i18n_key ? I18n.translate(pageConfig.i18n_key, pageConfig.name) : pageConfig.name;

    UI.elements.pageTitle.textContent = title || 'Zram WebUI';
  }
  ...
  static registerPageActions(pageName, actions) {
    ...
    UI.pageActions.set(pageName, validActions);
  ...
  static updatePageActions(pageName) {
    const actionsContainer = UI.elements.pageActions;
    if (!actionsContainer) return;

    // 清理之前的事件监听器
    UI.activeActions.forEach(id => {
      const button = document.getElementById(id);
      if (button) {
        button.replaceWith(button.cloneNode(true));
      }
    });
    UI.activeActions.clear();

    const actions = UI.pageActions.get(pageName) || [];
    ...
    actions.forEach(action => {
      ...
      if (button && action.onClick) {
        UI.activeActions.add(action.id);
        ...
        if (action.disabled) {
          ...
          updateDisabled(); // 初始检查
          // 添加到更新队列
          if (UI.buttonStateInterval) {
            clearInterval(UI.buttonStateInterval);
          }
          UI.buttonStateInterval = setInterval(updateDisabled, 1000);
        }
      }
    });
  }
  ...
  static clearPageActions(pageName) {
    ...
    // 移除页面按钮配置
    if (pageName) {
      UI.pageActions.delete(pageName);
    }
  }

  // 显示错误信息
  static escapeErrorText(text) {
    return String(text)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
  ...
  static showError(title, error) {
    ...
    const detailsHtml = stack
      ? `<details style="margin-top: 12px; white-space: normal;">
                    <summary style="cursor: pointer; user-select: text;">错误详情</summary>
                    <pre style="margin-top: 12px; white-space: pre-wrap; word-break: break-word; overflow-x: auto;">${UI.escapeErrorText(stack)}</pre>
                </details>`
      : '';
    ...
    UI.elements.mainContent.innerHTML = `
            <div class="page-container">
                <div class="error-container">
                    <h2>${title}</h2>
                    <p>${UI.escapeErrorText(message)}</p>
                    ${detailsHtml}
                </div>
            </div>
        `;
  ...
  static updateLayout() {
    ...
    const header = UI.elements.header;
    const pageActions = UI.elements.pageActions;
    const appNav = UI.elements.appNav;
    ...
    if (isLandscape) {
      ...
      if (appNav) {
        ...
        if (navContent) {
          ...
          if (systemActionsContainer && !systemActionsContainer.children.length) {
            systemActionsContainer.remove();
          }
        }
      }
    } else {
  ...
  static updateHeaderTransparency() {
    ...
    const _contentHeight = mainContent.scrollHeight;
    const _viewportHeight = mainContent.clientHeight;

    // 跟踪上次滚动位置
    if (!UI.lastScrollPosition) {
      UI.lastScrollPosition = scrollTop;
    }

    // 计算滚动方向 (1=向下, -1=向上)
    const scrollDirection = scrollTop > UI.lastScrollPosition ? 1 : -1;
    UI.lastScrollPosition = scrollTop;
