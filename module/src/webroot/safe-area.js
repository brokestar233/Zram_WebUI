...
Core.onDOMReady(() => {
  ...
  if (header) {
    ...
    logToConsole(
      `Header styles - position: ${headerStyles.position}, margin-top: ${headerStyles.marginTop}, padding-top: ${headerStyles.paddingTop}, box-sizing: ${headerStyles.boxSizing}`
    );
  }
  handleSafeArea();
});
...
function handleSafeArea() {
  ...
  if (mainContent) {
    ...
    logToConsole(
      `Main content styles - margin-top: ${mainContentStyles.marginTop}, padding-top: ${mainContentStyles.paddingTop}, box-sizing: ${mainContentStyles.boxSizing}`
    );
  }
  ...
  if (isCoreAvailable) {
    ...
    // 执行命令前先记录
    logToConsole(
      'Executing command: dumpsys window | grep -E "InsetsFrameProvider.*mandatorySystemGestures.*mMinimalInsetsSizeInDisplayCutoutSafe"'
    );
...
function updateStatusBarHeight() {
  if (isCoreAvailable) {
    Core.execCommand(
      'dumpsys window | grep -E "InsetsFrameProvider.*mandatorySystemGestures.*mMinimalInsetsSizeInDisplayCutoutSafe"'
    )
      .then(result => {
        ...
        let match = result.match(
          /mMinimalInsetsSizeInDisplayCutoutSafe=Insets{left=\d+, top=(\d+), right=\d+, bottom=\d+}/
        );
        if (match && match[1]) {
...
function applyStatusBarHeight(height) {
  ...
  logToConsole('Applying status bar height: ' + finalHeight);
  const heightPx = finalHeight + 'px';
  const offsetPx = finalHeight + 15 + 'px';
  const header = document.querySelector('.app-header');
  const mainContent = document.getElementById('main-content');
